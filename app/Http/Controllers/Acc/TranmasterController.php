...
class TranmasterController extends Controller {
  ...
  public function help()
	...
	{	
		...
		if(strpos($uc, '/')!=0):
			$rr=explode('/',$uc);
			$find=array('tdate'=>$rr[1],'vnumber'=>$rr[0]);
	...
	{	
		...
		$tranmasters = Tranmasters::where('com_id',$com_id)->where('check_action','')->where('check_id',Auth::id())->where('tmamount','>','0')->latest()->take(50)->get();
        $langs = $this->language();
	...
	{	
		...
		->where('appr_id',Auth::id())->oldest()->take(50)->get();
        $langs = $this->language();
...
public function getStyle($id)
 ...
 public function subhead()
	...
	public function voucher_print($id){  
		...
		$ttype=''; isset($find) && $find->id >0 ? $ttype=$find->ttype : $ttype='';
		if ($ttype=='Journal'):
			$pdf = \PDF::loadView('acc.tranmaster.jv_print', compact(['tranmaster', 'langs','users', 'reqs','acccoa']))->setOption('minimum-font-size', 10);
 ...
 public function tbalance()
	{
	 ...
	 //$advance=Trandetails::select(DB::raw('sum(amount) as amount, sh_id'))->where('acc_id',$adv_id)->groupBy('sh_id')->where('com_id',$com_id)->where('sh_id','>','0')->get();
		...
		$tran = Trandetails::join('1acc_coas','acc_trandetails.acc_id','=','acc_coas.id')->select(DB::raw('sum(amount) as amount, acc_id'))->where('acc_trandetails.com_id',$com_id)->groupBy('acc_id')->get();
        $langs = $this->language();
		return view('acc.tranmaster.tbalance', compact(['tran', 'langs','acccoa']));
	}
