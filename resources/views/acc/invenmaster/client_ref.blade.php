            ...
            <table id="buyerinfo-table" class="table table-bordered table-striped">
                ...
                <?php 
						if (isset($data['client_id']) && $data['client_id'] > 0): 
							...
							->whereBetween('idate', [$data['dfrom'], $data['dto']])->orderBy('ref')->get();	
						endif;
						...
						$op_bal=DB::table('acc_invenmasters')->select(DB::raw('sum(qty) as qty'))
							...
							$balance=$op_bal->qty;
							$unit_name=''; $ttl_rcvds='' ; $ttl_issues=''; $prodx_name ='';
                ...
                @foreach($sale as $item)
					...
					?>
                        ...
                        <?php 
							...
							//production
							if($item->prod_id>0): 
								$prodx=DB::table('acc_products')->where('id',$item->prod_id)->first();						
								 isset($prodx) && $prodx->id > 0 ? $prodx_name=' ,'.$prodx->name : $prodx_name=''; //echo $prod_name.'123';
							endif;
                         ...
                         <tr>
                            ...
                            <td>{{ $itype }} </td>
                            <td>{{ $note.$for_name.$ref.$prodx_name }} </td>
