    ...
    <div class="table-responsive">
        ...
        <?php 
			...
			if (isset($data['acc_id']) && $data['acc_id']>0):
				...
				echo '<tr><td class="text-center" colspan="2"><h4>General Ledger</h4><h >LC  Number: '.$lc_number.'</h5></td></tr>';
			endif;
		...
		?>
            ...
            <table id="buyerinfo-table" class="table table-bordered table-striped">
                <thead>
                <tr><td colspan="8"><a href="{!! url('/lcinfo/ledger?flag=lcfilter') !!}"> Filter  </a>
					<?php 
					...
					?>
                    @if ($flags=='lcfilter')
                           {!! Form::open(['url' => 'lcinfo/lcfilter', 'class' => 'form-horizontal']) !!}
                ...
                <?php 
						...
						if (isset($data['lc_id']) && $data['lc_id'] > 0 && $data['acc_id']==''): 
							...
							->join('acc_trandetails', 'acc_coas.id', '=','acc_trandetails.acc_id')
							->where('acc_trandetails.lc_id', $data['lc_id'])
							->where('acc_trandetails.acc_id', $data['acc_id'])->groupBy('acc_coas.id')->get();
						endif;
