...
@section('contentheader_title', 	  ' Marketing Team-wise Sales')
 ...
 <div class="box" >
    <div class="table-responsive">
        ...
        <?php 
			...
			$mteam=DB::table('acc_mteams')->where('id',$data['acc_id'])->where('com_id',$com_id)->first();
			isset($mteam) && $mteam->id >0 ? $mteam_name=$mteam->name : $mteam_name='';
		
			if (isset($data['acc_id']) && $data['acc_id']>0):
				// for single account
				$data['acc_id']> 0 ? $invoice='' : $invoice='';
				echo '<tr><td ><h3 class="pull-left">Sale Details</h3></td>
				<td class="text-right" ><h3 aling="right">For :'.$mteam_name.'</h3><h5 >'.$data['dfrom'].' to '.$data['dto'].'</h5></td></tr>';
			else:
		...
		?>
            ...
            <table id="buyerinfo-table" class="table table-bordered table-striped">
                <thead>
                <tr><td colspan="8"><a href="{!! url('/salemaster/mteam?flag=filter') !!}"> Filter  </a>
					<?php 
					...
					?>
                    @if ($flags=='filter')
                           {!! Form::open(['url' => 'salemaster/mfilter', 'class' => 'form-horizontal']) !!}
            
                            <div class="form-group">
                                {!! Form::label('acc_id', $langs['mt_id'], ['class' => 'col-sm-3 control-label']) !!}
                                <div class="col-sm-6"> 
                                    {!! Form::select('acc_id', $mteams, null, ['class' => 'form-control']) !!}
                                </div>    
                            </div>
                            <div class="form-group">
