...
@section('contentheader_title', 	  ' Sale Challan')
...
<style>
 ...
 <div class="box" >
        <div class="box-header">
            <a href="{{ url('salemaster/challan_print') }}" title="{{ $langs['print'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-print"></i></a>
            <a href="{{ url('salemaster/challan_pdf') }}" title="{{ $langs['download'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-download"></i></a>
            <a href="{{ url('salemaster/challan_pdf') }}" title="{{ $langs['pdf'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-file-pdf-o"></i></a>
            <a href="{{ url('salemaster/challan_excel') }}" title="{{ $langs['excel'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-file-excel-o"></i></a>
            <a href="{{ url('salemaster/challan_csv') }}" title="{{ $langs['csv'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-file-excel-o"></i></a>
            <a href="{{ url('salemaster/challan_word') }}" title="{{ $langs['word'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-file-word-o"></i></a>
        </div><!-- /.box-header -->
    ...
    <div class="table-responsive">
        ...
        <?php 
			...
			if (isset($data['acc_id']) && $data['acc_id']>0):
				...
				<td class="text-right"  id="td"><h4 aling="right">Challan No: '.$invoices[$data['acc_id']].'/ch</h4><h4>Date: '.$sales->sdate.'</h4></td></tr>';
			else:
		...
		?>
            ...
            <table id="buyerinfo-table" class="table">
                <thead>
                 ...
                 <tr><td colspan="6"> Name: {{ $client_name }} </td></tr>
                 <tr><td colspan="6"> Address: {{ $client_address }}</td></tr> 
                    <tr>
                ...
                @foreach($sale as $item)
                 	...
                 	<?php
					$username=$item->user_id >0 ? $users[$item->user_id] : ''; //echo  $users[$item->check_id];
					$checkname=$item->check_id>0 && $item->check_action==1 ? $users[$item->check_id] : 'waiting';
					//DB::table('a')->first();
					...
					?>
                        ...
                        <tr><td colspan="6" class=" text-left">Inwords : </td></tr> 
