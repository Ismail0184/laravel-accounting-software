...
@section('contentheader_title', 	  ' Product-wise Sales')
 ...
 <div class="box" >
    <div class="table-responsive">
        ...
        <?php 
			...
			$product=DB::table('acc_products')->where('id',$data['acc_id'])->where('com_id',$com_id)->first();
			...
			if (isset($data['acc_id']) && $data['acc_id']>0):
				...
				<td class="text-right" ><h3 aling="right">For :'.$product_name.'</h3><h5 >'.$data['dfrom'].' to '.$data['dto'].'</h5></td></tr>';
			else:
		...
		?>
            ...
            <table id="buyerinfo-table" class="table table-bordered table-striped">
                <thead>
                <tr><td colspan="8"><a href="{!! url('/salemaster/product?flag=filter') !!}"> Filter  </a>
					<?php 
					...
					?>
                    @if ($flags=='filter')
                           {!! Form::open(['url' => 'salemaster/pfilter', 'class' => 'form-horizontal']) !!}
            
                            <div class="form-group">
                                {!! Form::label('acc_id', $langs['prod_id'], ['class' => 'col-sm-3 control-label']) !!}
                                <div class="col-sm-6"> 
                                    {!! Form::select('acc_id', $products, null, ['class' => 'form-control']) !!}
                                </div>    
                            </div>
                            <div class="form-group">
                ...
                <?php 
						if (isset($data['dfrom']) && $data['acc_id']!=''): 
							...
							->where('acc_saledetails.com_id',$com_id)->get();	
						endif;
                ...
                @foreach($sale as $item)
					...
					//$user=$item->user->name;
					$check_has='';
					$details=DB::table('acc_saledetails')->where('com_id',$com_id)->where('sm_id', $item->sm_id)->where('item_id',$item->item_id)->get(); 
