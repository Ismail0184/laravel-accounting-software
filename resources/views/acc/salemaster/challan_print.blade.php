...
<style>
	...
	#col {  min-height:100px; padding-top:50px; width:23%; float:left; padding:20px ; background-color:; margin:5px; font-size:10px}
 ...
 <div class="box" >
    ...
    <div class="table-responsive">
		...
		?>
        	<h1 align="center" class="cname">{{ $com->name }}</h1>
            <h5 class="text-center adr ">{{ $com->oaddress }}</h5>
        </td><td></td></tr>
        <?php 
			...
			if (isset($data['acc_id']) && $data['acc_id']>0):
				...
				echo '<tr><td ><h2 class="pull-left rpt">Challan</h2></td><td></td>
				<td class="text-right"  id="td"><h4 aling="right" class="rpt">Challan No: '.$invoices[$data['acc_id']].'/ch</h4><h4 class="rpt">Date: '.$sales->sdate.'</h4></td></tr>';
			else:
				// for multiple account
				echo '<tr><td class="text-center" colspan="2" class="rpt"><h4>Invoice</h4><h5 ></h5></td></tr>';
			endif;
