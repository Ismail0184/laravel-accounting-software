 ...
 <div class="box" >
    <div class="table-responsive">
        ...
        <?php 
			...
			$client=DB::table('acc_clients')->where('com_id',$com_id)->where('id',$data['acc_id'])->first();
			$clientname=''; isset($client) && $client->id > 0 ? $clientname=$client->name : $clientname='';
			...
			if (isset($data['acc_id']) && $data['acc_id']>0):
				...
				<td class="text-right" ><h3 aling="right">'.$clientname.'</h3><h5 >'.$data['dfrom'].' to '.$data['dto'].'</h5></td></tr>'; //'.$invoices[$data['acc_id']].'
			else:
		...
		?>
            ...
            <table id="buyerinfo-table" class="table table-bordered table-striped">
                <thead>
                <tr><td colspan="8"><a href="{!! url('/purchasemaster/client?flag=filter') !!}"> Filter  </a>
					<?php 
					...
					?>
                    @if ($flags=='filter')
                           {!! Form::open(['url' => 'purchasemaster/clfilter', 'class' => 'form-horizontal']) !!}
            
                            <div class="form-group">
                                {!! Form::label('acc_id', $langs['client_id'], ['class' => 'col-sm-3 control-label']) !!}
                                <div class="col-sm-6"> 
                                    {!! Form::select('acc_id', $clients, null, ['class' => 'form-control','required']) !!}
                                </div>    
                            </div>
                            <div class="form-group">
