...
<body>
 ...
 <div class="row center-block" >
    <div class="table-responsive">
        ...
        <tr><td class="text-center"><h1>Order Chain Management Services</h1></td></tr>
        <tr><td class="text-center"><h5>Purchase Requisition List</h5></td></tr>
        </table>
        </div><!-- /.box-header -->
				...
				?>
                        @foreach($requisition as $item)
                         ...
                         <tr>
                            ...
                            <td>{{ $item->coa->name }}</td>
                            <td>{{ $item->currency->name }}</td><td class=" text-right">{{ $item->amount }}</td>
                            <td>{{ $item->check->name }}</td>
                            <td>{{ $item->approve->name }}</td>
                         </tr>
                         <?php $ttl=0; $ttl += $item->amount; ?>
                        @endforeach   
