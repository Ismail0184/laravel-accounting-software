...
@section('contentheader_title', 	  'Import Details')
...
<style>
 ...
 <div class="box" >
    <div class="table-responsive">
		...
		<table  width="100%>
        <?php 
			isset($_GET['supplier_id']) && $_GET['supplier_id']> 0 ? Session::put('ssupplier_id', $_GET['supplier_id']) : '';
			...
			Session::has('sdto') ? 
			$data=array('supplier_id'=>Session::get('ssupplier_id'),'dfrom'=>Session::get('sdfrom'),'dto'=>Session::get('sdto')) : 
			$data=array('supplier_id'=>'','dfrom'=>date('Y-m-01'),'dto'=>date('Y-m-d'));
			
			if (isset($data['supplier_id']) && $data['supplier_id']>0):
				$supplier=DB::table('acc_suppliers')->where('com_id',$com_id)->where('id',$data['supplier_id'])->first();
				$supplier_name=''; isset($supplier) && $supplier->id > 0 ? $supplier_name=$supplier->name : $supplier_name='';
			// for single account
			echo '<tr><td ><h3 class="pull-left">Supplier-wise Import</h3></td>
				<td class="text-right" ><h3 aling="right">'.$supplier_name.'</h3><h5 >'.$data['dfrom'].' to '.$data['dto'].'</h5></td></tr>';
			else:
				// for multiple account
				echo '<tr><td class="text-center" colspan="2"><h3>Supplier-wise Import</h3><h5 ></h5></td></tr>';
			endif;
		...
		?>
            ...
            <table id="buyerinfo-table" class="table table-bordered table-striped">
                <thead>
                <tr><td colspan="6"><a href="{!! url('/importmaster/supplier?flag=filter') !!}"> Filter  </a>
                <?php 
					...
					?>
                    @if ($flags=='filter')
                           {!! Form::open(['url' => 'importmaster/sfilter', 'class' => 'form-horizontal']) !!}
				...
				{{-- */$x=0;/* --}}
                <?php 
						isset($data['supplier_id']) && $data['supplier_id']>0 ?
						$import=DB::table('acc_importmasters')
						->join('acc_lcimports','acc_importmasters.lcimport_id','=','acc_lcimports.id')
						->join('acc_importdetails','acc_importmasters.id','=','acc_importdetails.im_id')
						->whereBetween('idate', [$data['dfrom'], $data['dto']])
						->where('acc_importmasters.com_id',$com_id)->where('supplier_id', $data['supplier_id'])->groupBy('im_id')->get() : '';
						$amount =''; $im_id=''; $ttl_amts='';$ttl_amount=''; 
				?>
                @foreach($import as $item)
               
                 	<?php
					$im_id= $item->id; $ilc_id=$data['supplier_id'];
					...
					?>
                        ...
                        <?php 
							...
							$amount=DB::table('acc_trandetails')->where('com_id',$com_id)->where('ilc_id', $data['supplier_id'])->sum('amount'); 
							
							$ttl_amount+=$item->amount;
                        ...
                        @endforeach 
                @endforeach
                <?php  $ttl_amounts=number_format($ttl_amount,2); ?>
                <tr><td colspan="9"></td><td class="text-right">{{ $ttl_amounts }}</td></tr>
                </tbody>
            </table>
