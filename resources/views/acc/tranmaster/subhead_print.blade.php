...
<style>
	...
	.ledger_name { font-size:14px}
	.daterang { font-size:12px}
	.tables {font-size:10px}
        ...
        <?php 
			...
			if (isset($data['acc_id']) && $data['acc_id']>0):
				...
				echo '<tr><td ><h4 class="pull-left ledger_name"> For '.$s_head.' </h4></td>
				<td class="text-right" ><h3 aling="right" class="ledger_name">'.$acc_head.'</h3><h5 class="daterang">'.$data['dfrom'].' to '.$data['dto'].'</h5></td></tr>';
			else:
				// for multiple account
				echo '<tr><td class="text-center" colspan="2"><h5>General Ledger</h5><h5class="daterang" >'.$data['dfrom'].' to '.$data['dto'].'</h5></td></tr>';
			endif;
		...
		?>
            ...
            <table width="100%" class="tables table">
                <thead>
                    <tr>
                        <th class="col-md-2" id="dt">{{ $langs['tdate'] }}Hasan habib</th>
