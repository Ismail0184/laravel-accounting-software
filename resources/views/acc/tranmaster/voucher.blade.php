...
<style>
	#col {  min-height:100px; padding-top:50px; width:20%; float:left}
...
@section('main-content')
    <div class="container">
        ...
        <div class="box-header">
        	@if($tranmaster->check_action==1 || $tranmaster->techeck_action==1)
...
-->			@endif
            ...
            <table class="table table-bordered">
                ...
                <tr>
                    <th id="lebel" >{{ $langs['person'] }} :</th><th>{{ $tranmaster->person }}</th><th id="lebel">{{ $v_n }} :</th><th id="lebel2">{{ $tranwith_name }}</th>
            </tr>
        </table><br>
        ...
        <table class="table" id="details">
               ...
               <?php 
					...
					$coa_name=''; isset($coa) && $coa->id >0 ? $coa_name=$coa->name : $coa_name='';		
						...
						$project_name ='';
						$item->pro_id>0 ? 
						$project = DB::table('acc_projects')->where('com_id',$com_id)->where('id',$item->pro_id)->first(): '';
						isset($project) && $project->id>0  ? $project_name = ', for the project of '.$project->name : $project_name =''; //echo $project->name.'-hasan'; 


						$department_name='';
						$item->dep_id>0 ? 
						$dep = DB::table('acc_departments')->where('com_id',$com_id)->where('id',$item->dep_id)->first(): '';
						isset($dep) && $dep->id > 0 ?  $department_name = ', Department of '.$dep->name : $department_name =''; 
			   ...
			   ?>
                <tr>
                    ...
                    <td >{{ $coa_name }} {{ $project_name }} {{ $department_name }}</td>
                    <td id="cur">{{ isset($tranmaster->currency->name) ? $tranmaster->currency->name : '' }}</td>
