...
@section('htmlheader_title', 'Profit and Loss')

@section('contentheader_title', 'Profit and Loss')
...
@section('main-content')
 ...
 <div class="box" >
    ...
    <a href="{{ url('tranmaster/profitloss_print') }}" title="{{ $langs['print'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-print"></i></a>
    <a href="{{ url('tranmaster/profitloss_pdf') }}" title="{{ $langs['download'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-download"></i></a>
    <a href="{{ url('tranmaster/profitloss_pdf') }}" title="{{ $langs['pdf'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-file-pdf-o"></i></a>
    <a href="{{ url('tranmaster/profitloss_excel') }}" title="{{ $langs['excel'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-file-excel-o"></i></a>
    <a href="{{ url('tranmaster/profitloss_csv') }}" title="{{ $langs['csv'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-file-excel-o"></i></a>
    <a href="{{ url('tranmaster/profitloss_word') }}" title="{{ $langs['word'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-file-word-o"></i></a>
        ...
        <?php 
			...
			echo '<tr><td ><h3 class="pull-left">Group Account</h3></td>
				...
				echo '<tr><td class="text-center" colspan="2"><h4>Profit and Loss Account</h4><h5 >'.$data['dfrom'].' to '.$data['dto'].'</h5></td></tr>';
			endif;
		...
		?>
            ...
            <table id="buyerinfo-table" class="table table-bordered ">
                <thead>
                <tr><td colspan="8"><a href="{!! url('/tranmaster/profitloss?flag=filter') !!}"> Filter  </a>
                <?php 
					...
					?>
                    @if ($flags=='filter')
                           {!! Form::open(['url' => 'tranmaster/pfilter', 'class' => 'form-horizontal']) !!}
                  ...
                  @foreach($tran as $item)
					 ...
					 //DB::table('acc_coas1')->get();
						...
						isset($find->acc_id) ? $flag=1 : $flag=0; //echo $flag;
								...
								$trad=DB::table('acc_coas')->where('com_id',$com_id)->where('name','Trading Account')->first();
								$trad_id=''; isset($trad) && $trad->id>0 ? $trad_id=$trad->id : $trad_id='';
								...
								->where('tdate','<=',$data['dto'])
								->where('acc_tranmasters.com_id',$com_id)
								->whereIn('topGroup_id',[$man_id,$trad_id])
								->sum('acc_trandetails.amount') ; 
								$pls> 0 ? $pl_text="Balance" : $pl_text="Balance";
					...
					?>
                    	<!-- Group Head-->
                  		<tr>
                            <td id='grp'>{{ $pl_text }} (Transfered from Trading Account)</td>
                        ...
                        <?php 
							...
							?>
                                  <tr>
                                    <td style="padding-left:30px"><a href="{{ url('/tranmaster/ledger?acc_id='.$item->id) }}">{{ $item->name }}</a> </td>
                        ...
                        <tr>
                            ...
                            <td id="cur"></td><td class=" text-right"></td>
                                   ...
                                   <!---------------second----------------->
                                    ...
                                    ?>
                                          <tr>
                                            <td style="padding-left:60px"><a href="{{ url('/tranmaster/ledger?acc_id='.$item->id) }}">{{ $item->name }}</a></td>
                                            <td id="cur">{{ $dcur }}</td><td class=" text-right">{{ $debit }}</td>
                                            <td id="cur">{{ $ccur }}</td><td class=" text-right">{{ $credit }}</td>
                                      ...
                                      <tr>
                                        ...
                                        <td id="cur"></td><td class=" text-right"></td>
                                                    ...
                                                    ?>
                                                      <tr>
                                                        <td style="padding-left:90px"><a href="{{ url('/tranmaster/ledger?acc_id='.$item->id) }}">{{ $item->name }}</a></td>
                                                        <td id="cur">{{ $dcur }}</td><td class=" text-right">{{ $debit }}</td>
                                                        <td id="cur">{{ $ccur }}</td><td class=" text-right">{{ $credit }}</td>
														...
														if (isset($item->atype) && $item->atype=='Group' && $flag==1):
														 ...
														 <tr>
															...
															<td id="cur"></td><td class=" text-right"></td>
                                                                    ...
                                                                    ?>
                                                                      <tr>
                                                                        <td style="padding-left:120px"><a href="{{ url('/tranmaster/ledger?acc_id='.$item->id) }}">{{ $item->name }}</a></td>
                                                                        <td id="cur">{{ $dcur }}</td><td class=" text-right">{{ $debit }}</td>
                                                                        <td id="cur">{{ $ccur }}</td><td class=" text-right">{{ $credit }}</td>
			...
			<div class="box-header">
                ...
                <tr><td class="text-left">Source: Transaction->Profit and Loss</td><td class="text-right">Report generated by: </td></tr>
                </table>
            </div><!-- /.box-header -->
     </div>
</div>
