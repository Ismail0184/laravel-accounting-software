...
@section('contentheader_title', 'Pay Slipt')
...
.sign { padding-top:100px; }
.sname {  border-top:1px solid #300; min-height:60px; padding:10px; margin:3px; width:200px}
...
use  App\Models\Acc\Salaries;
		...
		function createSalary($id, $m, $y, $asalary, $loan){
			...
			$employee=DB::table('acc_empolyees')->where('id',$id)->first();
			$employee_name='';isset($employee) && $employee->id>0 ? $employee_name=$employee->name : $employee_name='';
?>
        ...
        <h4 class="text-center rpt">{{ $deprt_name }}</h4>
        <h3 class="text-center cname">Pay Slip</h3>
        <h4 class="text-center rpt"><b>Name</b> :{{ $employee_name }}</h4>
...
-->                        <th rowspan="2" class="text-right">{{ $langs['basic'] }}</th>
                    ...
                    <tr>
                        ...
                        <th class="text-right">{{ $langs['other'] }}</th>
                ...
                {{-- */$x=0;/* --}}
                <?php 
					$npay=''; $ttl_salary=''; $ttl_np=''; $ttl_asalary=''; $ttl_loan=''; $ttl_deduct=''; $ttl_absence=''; $ttl_tpsalary=''; $ttl_loans=''; $ttl_deducts=''; $ttl_ad=''; $ttl_mobile='';$ttl_lunch=''; $vno='#'; $ttl_esf='';$ttl_other='';
				?>
                @foreach($department as $item)
                    {{-- */$x++;/* --}}
                    <?php 

						$otime=fld_value($item->id, $m_id, $year, 'otime'); $otimes=''; $otime>0 ? $otimes=number_format($otime) : $otimes='';
						$hday=fld_value($item->id, $m_id, $year, 'hday'); $hdays=''; $hday>0 ? $hdays=number_format($hday) : $hdays='';
						...
						asalary($item->sh_id, $m_id, $year)>0 ? $asalary=number_format(asalary($item->sh_id, $m_id, $year),2) : $asalary='';
...
-->                        <td class="text-right">{{ number_format($item->bsalary) }}</td>
                ...
                <tr>
                <tr><td class="text-left" colspan="10">
                <div class="sign">
                <div  class="sname">Authorized Signature</div>
                </div>
                </td><td class="text-right" colspan="9">
                <div class="sign">
                <div  class="sname pull-right" >Received By</div>
                </div>
