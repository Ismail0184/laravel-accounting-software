...
?>
    <div class="box">
            <a href="{{ url('empolyee/payslip_print/'.$id) }}" title="{{ $langs['print'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-print"></i></a>
...
<?php 
			...
			echo account_exist('ESF Account','Current Liabilities','Balance Sheet','Group');
            ...
            <table id="empolyee-table" class="table table-bordered table-striped">
                <thead>
                    <tr>
                        ...
                        <th rowspan="2" class="text-right">{{ $langs['tran'] }}</th>
                        <th rowspan="2" class="text-right">{{ $langs['mexp'] }}</th>
                        <th rowspan="2" class="text-right">{{ $langs['enter'] }}</th>
                    ...
                    <tr>
                        ...
                        <th class="text-right">{{ $langs['esf'] }}</th>
                        <th class="text-right">{{ $langs['ttl'] }}</th>
                    </tr>
                ...
                {{-- */$x=0;/* --}}
                <?php 
					$npay=''; $ttl_salary=''; $ttl_np=''; $ttl_asalary=''; $ttl_loan=''; $ttl_deduct=''; $ttl_absence=''; $ttl_tpsalary=''; $ttl_loans=''; $ttl_deducts=''; $ttl_ad=''; 
				?>
                @foreach($department as $item)
                    ...
                    <?php 
						...
						$esf=$item->bsalary*10/100;
						$deduct=asalary($item->sh_id, $m_id, $year)+eloan($item->sh_id)+$esf+$absence; ; 
                    ...
                    <tr>
                  	{!! Form::model($department, ['url' => ['salary/pay', $item->id], 'method' => 'UPDATE', 'class' => 'form-horizontal tranmaster']) !!}
                        ...
                        <td class="text-right">{{ number_format($esf) }}</td>
                ...
                <tr>
                    ...
                    <td colspan="" class="text-right">{{ $ttl_nps }}</td>
                </tr>
                </tbody>
            </table>
        </div>
