...
@section('contentheader_title', 'Transaction Check')
...
@section('main-content')
	...
	<?php	
		...
		isset($option) && $option->id> 0 ? $option_appr_id=$option->tappr_id : $option_appr_id=0;
                    ...
                    {{-- */$x++;/* --}}
                     <?php 
						...
						$item->tmamount>0 ? $item->tmamount=number_format($item->tmamount,2) : '';					 					 
						 ?>
                   <tr>
                  {!! Form::model($tranmasters, ['url' => ['tranmaster/checked', $item->id], 'method' => 'UPDATE', 'class' => 'form-horizontal tranmaster']) !!}
                       ...
                       <td id="amt">{{ $users[$item->user_id] }}</td>
                        <td id="tp">{!! Form::select('appr_id', $users, $option_appr_id, ['class' => 'form-control' , 'required', 'style'=>'width:200px']) !!}
