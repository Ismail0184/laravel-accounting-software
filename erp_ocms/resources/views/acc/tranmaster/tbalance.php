...
@section('htmlheader_title', ' Department Ledger')
...
<style>
 ...
 <div class="box" >
    ...
    <a href="{{ url('tranmaster/department_print') }}" title="{{ $langs['print'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-print"></i></a>
    <a href="{{ url('tranmaster/department_pdf') }}" title="{{ $langs['download'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-download"></i></a>
    <a href="{{ url('tranmaster/department_pdf') }}" title="{{ $langs['pdf'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-file-pdf-o"></i></a>
    <a href="{{ url('tranmaster/department_excel') }}" title="{{ $langs['excel'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-file-excel-o"></i></a>
    <a href="{{ url('tranmaster/department_csv') }}" title="{{ $langs['csv'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-file-excel-o"></i></a>
    <a href="{{ url('tranmaster/department_word') }}" title="{{ $langs['word'] }}" class="btn btn-primary pull-right btn-sm trash-btn"><i class="fa fa-file-word-o"></i></a>
        ...
        <?php 
			...
			if (isset($data['acc_id']) && $data['acc_id']>0):
				...
				echo '<tr><td ><h4 class="pull-left">'.$department.' Department</h4></td>
		...
		?>
            ...
            <table id="buyerinfo-table" class="table table-bordered table-striped">
                <thead>
                <tr><td colspan="8"><a href="{!! url('/tranmaster/department?flag=filter') !!}"> Filter  </a>
					<?php 
					...
					?>
                    @if ($flags=='filter')
                           {!! Form::open(['url' => 'tranmaster/depfilter', 'class' => 'form-horizontal']) !!}
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-6"> 
                                    {!! Form::select('dep_id', $departments, null, ['class' => 'form-control select2']) !!}
                                </div>    
                            </div>
                            <div class="form-group">
                ...
                <?php 
						...
						isset($data['dep_id']) && $data['dep_id'] > 0 ?  
						$tran=DB::table('acc_coas')->where('acc_coas.com_id', $com_id)->select('acc_coas.id as id')
						->join('acc_trandetails', 'acc_coas.id', '=','acc_trandetails.acc_id')->where('dep_id',$data['dep_id'])->groupBy('acc_coas.id')->get() :'';
