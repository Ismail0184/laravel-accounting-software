...
@section('contentheader_title', 	  ' Client-wise Sales')
 ...
 <div class="box" >
    <div class="table-responsive">
        ...
        <?php 
			...
			Session::has('cldto') ? 
			$data=array('acc_id'=>Session::get('clacc_id'),'dfrom'=>Session::get('cldfrom'),'dto'=>Session::get('cldto')) : 
			$data=array('acc_id'=>'','dfrom'=>date('Y-m-01'),'dto'=>date('Y-m-d'));
			$client=DB::table('acc_clients')->where('id',$data['acc_id'])->where('com_id',$com_id)->first();
			...
			if (isset($data['acc_id']) && $data['acc_id']>0):
				...
				<td class="text-right" ><h3 aling="right">For :'.$client_name.'</h3><h5 >'.$data['dfrom'].' to '.$data['dto'].'</h5></td></tr>';
			else:
		...
		?>
            ...
            <table id="buyerinfo-table" class="table table-bordered table-striped">
                <thead>
                <tr><td colspan="8"><a href="{!! url('/salemaster/client?flag=filter') !!}"> Filter  </a>
					<?php 
					...
					?>
                    @if ($flags=='filter')
                           {!! Form::open(['url' => 'salemaster/clfilter', 'class' => 'form-horizontal']) !!}
            
                            <div class="form-group">
                                {!! Form::label('acc_id', $langs['client_id'], ['class' => 'col-sm-3 control-label']) !!}
                                <div class="col-sm-6"> 
                                    {!! Form::select('acc_id', $clients, null, ['class' => 'form-control']) !!}
                                </div>    
                            </div>
                            <div class="form-group">
                ...
                @endforeach
						...
						?> 
                        <tr><td colspan="7" class=" text-right">Total</td><td id="cur"><td class=" text-right">{{ $ttls }}</td></tr> 
                            </td></tr>
